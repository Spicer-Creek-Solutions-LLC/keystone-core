...
| Record a pending enrollment | Token identifier, agent name, expiry, state `pending`, and the bootstrap identity's public key |
...
identity is the one minted for that token; and the request is signed by the
envelope-signing key whose public half it carries.
...
| S1 | `issued` | Server validates (§ 5), mints the permanent user JWT against the agent's public NKey with `AST-16`, records it, replies on the token-scoped subject |
| S2 | `issued` | **Agent** writes the credential file: write to a temporary file in the same directory, `fsync`, `rename` atomically, `fsync` the directory, mode `0600` |
...
| `ARCH-JOB-003` | § 10 — **registered**; the ledger's schema is P08's |
...
format, signing and encryption operations, and `RSK-12` (**P05**); job lifecycle
and ledger semantics (**P06**); audit record schema and retention (**P08**);
