...
| Risk | Outcome |
|---|---|
| `RSK-11` | **Resolved.** § 4 signs every envelope class, cancellation included, so a holder of the command publisher credential alone cannot produce an accepted cancellation. **This closes the exposure and not the invariant gap** — see § 11 |
...
| `RSK-6` | **Renewed, narrowed.** § 8 fixes the header set and § 7 enumerates the accepted leakage per class, so what the broker learns is now stated rather than inferred. It is not reduced: signing does not narrow metadata leakage, and no class's leakage exceeds what § 6 already conceded |
