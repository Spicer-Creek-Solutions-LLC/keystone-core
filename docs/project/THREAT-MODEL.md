...
| `AST-17` | TLS CA private key — issues the broker certificate every client verifies (`ADR-0002` § 12, `G49`). **Never in the deployment**: the generator returns it to its caller | `TD-BRK` — held by the administrator, outside every process the deployment runs |
| `AST-18` | Broker TLS private key — the broker's certificate identity (`ADR-0002` § 12, `G49`) | `TD-BRK` |
...
| `THR-03` | Bootstrap access survives enrollment and is reused later | `ACT-7` | `AST-2` | 5.1 | The server refuses the spent token at S4, and the broker refuses the bootstrap credential from its expiry, minutes later (`ARCH-NATS-004`, RFC 0005). **Bounded, not closed**: until expiry the credential can do almost nothing, and `RSK-15` accepts that window |
...
| `THR-55` | A stolen broker TLS key, or a certificate issued with the stolen CA key, lets an attacker **impersonate the broker** to clients | `ACT-6`, `ACT-10` | `AST-17`, `AST-18` | all | `RSK-16`. The CA key is never written to the deployment and the broker key is mode `0600` in `TD-BRK` (`G49`); a broker certificate lives a year by default. **The impersonator gains `ACT-5`'s view and no more**: payloads are signed end to end and command and result payloads are encrypted (`ARCH-NATS-006`), so it can relay, drop or delay and cannot forge or read them |
...
| `ARCH-NATS-006` | `THR-10`, `THR-21`, `THR-24`, `THR-33`, `THR-50`, `THR-55` |
...
|---|---|---|---|
| `AST-2` bootstrap credential | Not rotated; it exists for one enrollment | **Not revoked at runtime** (RFC 0005): the server refuses the spent token and the broker enforces the credential's short expiry. The operator key can revoke it offline | Defined crash-recovery path at every enrollment stage |
| `AST-3` permanent agent credential | **Re-enrollment** (`ADR-0003` § 10, § 11) — `RSK-7`, resolved | **Decided, not built** (RFC 0005 § 3): the server refuses the identity at once and declines to renew a short-lived credential, so the broker ends it at expiry. The operator key revokes it offline in an emergency. The effect on in-flight jobs is P03 and P06 | Re-enrollment |
...
| `AST-17` TLS CA key | **No procedure specified** — `RSK-16`, C13. Rotating it re-anchors every client | **None**: a CA is trusted by being configured, and the only way out is replacing the anchor on every client | Issue a new CA and broker certificate, and redistribute the anchor — outside the product |
| `AST-18` broker TLS key | Reissue from `AST-17` before the certificate expires — **no procedure specified**, `RSK-16`, C13 | **None**: clients check no CRL or OCSP, so a stolen key is usable until its certificate expires | Reissue the certificate and restart the broker |
...
connection, refusal of the spent token and expiry of the bootstrap credential,
with a crash-recovery path per stage. It said *revoke, verify* until RFC 0005
found the server's key could do neither. **Everything
...
| `RSK-15` | Bootstrap access outlives enrollment until the token expires (`THR-03`) | Project maintainer | The server cannot revoke a NATS user with the key it holds, and the alternatives — an operator signing key on the server, or a separate revoker — cost more than the window is worth (RFC 0005) | After S4 the server refuses the spent token and audits every attempt, and the credential's only grants are its own two `ks.enroll.<token>.*` subjects: it reaches no agent, job, stream or other token. The broker ends it at expiry, a lifetime of minutes that `ADR-0003` § 2 keeps short. **The whole exposure is that lifetime**, which C05 chooses | **Accepted at G48.** 2027-03-23, or C05 |
| `RSK-16` | A stolen broker TLS key or TLS CA key allows **broker impersonation**, and nothing revokes a broker certificate (`THR-55`, `AST-17`, `AST-18`) | Project maintainer | Revocation checking needs CRL or OCSP infrastructure Generation 2 does not run, and a rotation procedure is operational work, which is C13's as `RSK-13`'s is. The exposure is bounded by what an impersonator can do: `ACT-5`'s view, because payloads are signed and encrypted end to end | The CA key never enters the deployment and is the administrator's to keep in an HSM, a KMS, a vault or offline media; the broker key is mode `0600`; the broker certificate lives a year by default; TLS 1.3, TLS-first, verification by name that no client can disable (`ADR-0002` § 12, `G49`) | **Accepted at G49.** 2027-03-23, or C13 |
...
**Residual risks record a decision, not a measurement.** Every `RSK-*` row says
the maintainer accepted a risk and why. None says the risk is
small.
