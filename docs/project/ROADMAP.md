...
- **Partially landed (PR-A)**: `cmd/kscore-server` now constructs the clustering stack at boot (`cluster.go` `startCluster`, gated on `cluster.enabled`) and registers `ClusterGRPCServer` on the operator surface with live `Leader`/`Members`/`Rebalancer`/`ShardStore`/`LeaderWatch` seams + the `MembershipManager`-backed `Evictor`; the REST handler gets real `ClusterProviders` (Leader/Members/Rebalance/Backup), so cluster routes go 503→200 (Status excepted). **Still deferred (PR-B)**: the dedicated mTLS server↔server CoordinationService listener, the `CoordinationClient` pool/switchover, the `GracefulShutdown` SIGTERM hookup + real `StopAccepting`, and the HealthMonitor-backed REST `Status` provider (`/cluster/status` stays 503 until then).
