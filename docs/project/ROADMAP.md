...
- **Priority**: gate-v1.0
- **What**: Epic 16 task 12 ships `internal/webhook/outbound.Manager` — push-driven (`Handle(ctx, events.Event)`); glob-matches each event against every enabled subscription's filter list via stdlib `filepath.Match`; fans out async through a bounded semaphore + `sync.WaitGroup`; persists per-attempt `DeliveryRecord`s via the task-11 `SubscriptionStore`; calls a `Dispatcher` seam (task 13 implements). The full pipeline (subscription cache + refresh, oversize handling, fan-out, Stop drain) is unit-tested with a fake dispatcher, and `config.WebhookOutboundConfig` carries the §4.14 keys plus the task-12 tunables (`max_concurrent_deliveries`, `refresh_interval`) — `webhook.outbound.enabled` defaults off (opt-in, the `gitops.webhook` precedent). It is **not yet constructed at `cmd/kscore-server` boot**: nothing subscribes `events.EventSubscriber` to route `kscore.<cluster>.events.>` deliveries into `Manager.Handle`, the live `Store` is still the in-memory default (task 11 ships both impls), and the Dispatcher (task 13) plus RetryQueue (task 14) + circuit breaker (task 15) decorators must layer in. REST/CLI (task 16) is also unwired. Until boot lands, the package surface is unit-tested only; a running kscore-server emits no outbound webhooks.
- **Why deferred**: same dependency-light / dark-until-boot root as the other Epic 16 boot entries — the Manager is push-driven precisely so the package stays free of the NATS / events subscriber API, and the boot wiring (subscribe + route + select Memory vs SQLite store + construct Dispatcher with retry + circuit-breaker decorators) lands in one coherent place once tasks 13-16 are in.
- **Acceptance for unblock**: kscore-server boot constructs the Manager over the SQLite `SubscriptionStore`, wraps the Dispatcher with the task-14 RetryQueue + task-15 circuit breaker, subscribes `events.EventSubscriber` to `kscore.<cluster>.events.>` and routes each delivery to `Manager.Handle`; the `/api/v1/webhooks/subscriptions` REST (task 16) is wired with the same Manager so its post-CRUD `Refresh` call lights up new subscriptions immediately.
- **References**: `internal/webhook/outbound/{types,store,store_sqlite,manager}.go`; `internal/config/webhook.go` (`WebhookOutboundConfig`); `internal/events/subscriber.go` (the subscriber API the boot path will consume); Epic 16 tasks 11-12 (landed), forward 13/14/15/16/17/18; companions: the GitOps webhook receiver + GitOps rollback boot-wiring entries.
