...
- **Priority**: gate-v0.5 (reboot disconnect-tolerance — remaining); v0.x (the rest). **Cross-distro reboot detection landed.**
...
- **Acceptance**: a `reboot: true` decl on RHEL 9 detects need via `dnf needs-restarting -r` exit code without an `/var/run/reboot-required` marker **(met — cross-distro detection landed)**; the agent delivers a `system.rebooted` event after a reboot Apply, with the boot-ID change as proof **(remaining gate-v0.5 — reboot disconnect-tolerance, a separate agent+events follow-up)**. v0.x: a `reboot: true` with `force: true` reboots unconditionally; a `locale: en_US.UTF-8` decl on Debian writes both `/etc/locale.conf` and `/etc/default/locale`; an `lc_all:` param round-trips into `LC_ALL=` in the locale conf; `state: absent` for `reboot` cancels a pending `shutdown -r`.
