...
- **What**: The generated promo-video pipeline landed (`assets/promo/`, `tools/promogen`, `make promo` / `update-promo` / `promo-check`; script and rationale in [`assets/promo/README.md`](../../assets/promo/README.md)). Deliberately deferred from the initial build:
  - **Move the scenario onto an agent host.** Terminal shots 3-5 currently apply server-side because the tapes predate `--target`. State apply now dispatches to agents, so the state shots -- and the `manage-secrets` docs clip, which applies `--target localhost` for the same reason -- can move onto an agent immediately; the blueprint shot still waits on the gate-v1.0 entry *Remote / distributed blueprint apply wiring*. The secrets clip has a second blocker of its own: it passes the credential as `--variable`, which is the workaround for the missing `secret` template function (see the gate-v1.0 entry *Agent-side secret resolution during state rendering*); once that lands the clip should show the state file referencing the secret directly. Once the scenario targets an agent, `scenario/state/web.yaml` should grow `package` + `service` declarations instead of being file-module-only (the server container is distroless, so there is nothing to converge today). Strictly better demo; the existing shot durations already accommodate it.
...
<!-- "Per-feature docs clips — multi-reel promo pipeline" landed. The
     manifest describes reels (per-reel budget / output / square_cut /
     docs_page, inheriting tolerance + resolution from defaults);
     promogen gained `reels`, `plan -reel <id>` and a docs_page embed
     check; pipeline/build.sh renders each reel and `make promo
     REEL=<id>` renders one. Five docs clips ship — fleet, remote-exec,
     secrets, gitops-rollback, outbound-webhooks — embedded via a
     `clip` shortcode on the getting-started index, secrets and gitops
     pages, and published into assets/promo/video/ by `make
     promo-publish` (served through the existing ../assets ->
     static/keystone Hugo mount).

     Two scenario additions were needed and are worth knowing about:
     the promo overlay builds the agent on debian-slim because the E2E
     image is distroless and `exec run` cannot find /bin/hostname, and
     it declares an nginx receiver on :8099 because nginx's default
     answers a POST with 405.

     Deliberately NOT done, and not tracked as follow-up unless someone
     wants them: clips for the remaining six demoable scenarios
     (blueprints, modules, audit query, health, GitOps webhook ingest),
     which are the less differentiating half. Entry retired; the
     pipeline and its docs live in assets/promo/README.md. -->
