...
- **References**: `internal/controlplane/grpc_coordination_server.go`; `internal/controlplane/coordination_client.go`; `api/proto/keystone/core/v1/coordination.proto`; Epic 13 tasks 12/13/15; companions: "Cluster leader-check boot wiring (kscore-server)", "Fencing guard wired around server write paths".
