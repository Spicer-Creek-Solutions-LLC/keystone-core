...
- **Acceptance for unblock**: a `K8sRolloutClient` client-go adapter (Deployment get + ReplicaSet revision history + rollout-undo) is wired so `kscore-gitops rollback --app web --strategy previous` against a real cluster rolls a Deployment back; the task-8 rollback Engine + `kscore-gitops` CLI/REST (task 10) are constructed at boot over a durable task-9 `RollbackStore`; `GetLastKnownGood` consults the task-9 verification-confirmed store rather than raw provider history.
- **References**: `internal/gitops/rollback/{executor,git,argocd,k8s,register,engine,state}.go`; `internal/gitops/rollback/gitexec` (go-git, landed); `internal/gitops/rollback/argoexec` (REST, landed); `pkg/statemachine` (engine FSM); PROJECT-DETAILS §3.2 (K8s row — client-go deferred), §4.13 external-clients; Epic 16 tasks 7-8 (landed), forward 9/10; companions: the GitOps webhook receiver + cluster/blueprint/runbook boot-wiring entries.
