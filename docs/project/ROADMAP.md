...
- **Priority**: gate-v0.5 work **complete** (filesystem resize landed for ext2/3/4); v0.x (the rest, incl. xfs/btrfs/f2fs resize).
- **What**: Epic 08 task 11 ships the `disk` module (single op: ensure `device:` has filesystem `fstype:` — `present` or `absent` — gated by an explicit `force: true` for any apply that would destroy existing data; mkfs binary resolved per fstype from a curated 9-entry catalog: ext2/3/4, xfs, btrfs, f2fs, vfat, exfat, swap). Reserved for v0.x:
  ...
  - **Filesystem resize** without destroying data: ~~`resize2fs` (ext2/3/4)~~ **landed** via `resize_fs: true` (idempotent: `dumpe2fs` block-count×size vs `blockdev --getsize64`, grow with `resize2fs <device>` only when the fs doesn't already fill the device). `xfs_growfs` (xfs), `btrfs filesystem resize`, `resize.f2fs` remain v0.x — they are mount-required (resize by mountpoint, not device) with per-fstype size math.
...
- **Acceptance**: a `disk` decl with `resize_fs: true` extends the (ext) fs to fill the device, idempotently **(met — ext fs-resize landed)**. v0.x: a `partition:` decl (in a new `partition` module, sibling to `disk`) creates `/dev/sdb1` at the declared start/size/type, idempotent against `parted -m print`; xfs/btrfs/f2fs resize; a `label: mylabel` decl on an existing ext4 device updates the label without re-format; a `luks: { passphrase: …, key_slot: 0 }` decl initialises LUKS on the device; `fstype: ntfs` round-trips via ntfs-3g; `on_mismatch: warn` reports the drift in the StateResult Comment but doesn't error.
