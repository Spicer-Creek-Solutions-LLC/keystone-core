...
- **Acceptance**: reels with independent duration budgets, validated by `make promo-check` and rendered by `make promo` — **landed**. Remaining: at least four per-feature clips exist and are embedded in their Hugo docs pages, with every clip's commands passing the `promogen tapes` guard.
