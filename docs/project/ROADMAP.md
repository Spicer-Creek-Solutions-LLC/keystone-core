...
- **What**: Epic 08 task 13 Layer C. **Landed**: all five distros (Debian 12, Ubuntu 22.04/24.04, Rocky 9, Alpine 3.19) run green via `make test-cross-distro`. Each boots in a **privileged container with its real init system** (systemd / OpenRC); a static CGO-free apply-harness (`test/e2e/state/harness/`, runs on glibc + musl) applies the smoke fixture **twice** and asserts the second pass is a zero-change no-op — so `package` (apt/dnf/apk) and `service` (systemd/OpenRC) are exercised against the live system, not mocked. The old `docker-compose.yml` scaffold was replaced by a `docker run -d` + `docker exec` orchestrator (`run.sh`) because the service module needs a booted init as PID 1, which compose's run-to-exit model can't provide. Docker auto-skip preserved.
- **Remaining**: the fixtures exercise `file` + `package` + `service` today (the modules with cross-distro backends). The other live-state modules join the fixtures **as their own gate-v0.5 backends land** — `user`/`group` need busybox `adduser`/`addgroup` (they currently shell out to shadow-utils, absent on Alpine); `firewall`/`firewalld`/`network`/`lvm`/`disk`/… ride their per-module entries above. So this entry stays open as the integration point, but the harness infrastructure + the package/service acceptance are done.
...
- **References**: `test/e2e/state/README.md`; `test/e2e/state/{run.sh,smoke.sh,smoke.systemd.yaml,smoke.openrc.yaml}`; `test/e2e/state/harness/`; the per-module gate-v0.5 entries above for `firewall`, `firewalld`, `security`, `system`, `lvm`, `disk`, `network`, `route`, `bond`, `bridge`, `vlan` (and the `user`/`group` busybox gap) name the per-distro acceptance each module must clear inside this harness as it lands.
