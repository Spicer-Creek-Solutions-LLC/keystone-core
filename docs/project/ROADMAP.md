...
- **Acceptance for unblock**: a `K8sRolloutClient` client-go adapter (Deployment get + ReplicaSet revision history + rollout-undo) is wired so `kscore-gitops rollback --app web --strategy previous` against a real cluster rolls a Deployment back; the rollback Engine (task 8) + `kscore-gitops` CLI/REST (task 10) are constructed at boot; `GetLastKnownGood` consults the task-9 verification-confirmed store rather than raw provider history.
- **References**: `internal/gitops/rollback/{executor,git,argocd,k8s,register}.go`; `internal/gitops/rollback/gitexec` (go-git, landed); `internal/gitops/rollback/argoexec` (REST, landed); PROJECT-DETAILS §3.2 (K8s row — client-go deferred), §4.13 external-clients; Epic 16 tasks 7 (landed), forward 8/9/10; companions: the GitOps webhook receiver + cluster/blueprint/runbook boot-wiring entries.
