...
- **Priority**: gate-v0.5 (service-catalog expansion — remaining); v0.x (the rest). **IPv6 on iptables landed.**
...
- **Acceptance**: `firewall.service: ssh` opens both v4 and v6 on every backend by default **(met — dual-stack iptables landed)**; the catalog accepts `dhcpv6-client` (firewalld backend) and `samba` (every backend, expanding to 4 sub-rules) **(remaining gate-v0.5 — service-catalog expansion)**. v0.x: `action: deny` round-trips on every backend; `chain: custom` / `family: ipv6` override on the iptables backend round-trips; `source: 10.0.0.0/8` filters the allow on every backend; the StateResult Comment carries `via <backend>`.
