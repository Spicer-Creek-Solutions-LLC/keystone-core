...
- **Remaining**: the fixtures exercise `file` + `package` + `service` + `user`/`group` + `hostname` + `disk` today. The other live-state modules join the fixtures **as their own gate-v0.5 backends land** — `firewall`/`firewalld`/`network`/`lvm`/… ride their per-module entries above. So this entry stays open as the integration point, but the harness infrastructure + the package/service/user/group/hostname/disk acceptance are done. Three distros were added to exercise the backends that landed after the original five — **openSUSE** (zypper) + **Arch** (pacman) for `package`, and **Devuan** (sysvinit) for `service`; sysvinit boots via a keep-alive container since its ops are script-based (no PID-1 init needed). These three are **not yet certified on a Docker host** — added best-effort (image tags, package/service names, boot commands validate on the first `make test-cross-distro` run; the matrix is failure-isolated per distro). `disk` runs in its own loop-device-backed phase (`smoke.disk.sh`): each fstype gets a blank-loop `mkfs` scenario and a small-fs-on-grown-device `resize` scenario (xfs/btrfs mounted, ext/f2fs offline), self-gating on the host `loop` module and per-fstype tool availability (btrfs/f2fs skip on Rocky 9). Caveats: on Alpine the BusyBox backend has no `usermod`/`groupmod`, so modifying an existing account's scalar fields (or a group's GID) returns `ErrModUnsupported` (the create/delete/lookup + supplementary-group paths the harness exercises are fully supported); and `hostname` requires detaching Docker's `/etc/hostname` bind-mount (`smoke.sh` runs `umount /etc/hostname`) since the bind-mount makes the inode unreplaceable (`EBUSY`) — a Docker-only step, no-op on a real host.
