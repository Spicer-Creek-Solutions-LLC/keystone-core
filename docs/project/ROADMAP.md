...
- **Acceptance for unblock**: a `K8sRolloutClient` client-go adapter (Deployment get + ReplicaSet revision history + rollout-undo) is wired so `kscore-gitops rollback --app web --strategy previous` against a real cluster rolls a Deployment back; the task-8 rollback Engine + `kscore-gitops` CLI/REST (task 10) are constructed at boot over the durable task-9 `rollback.SQLiteStore`; `GetLastKnownGood` consults the task-9 `verification.ResultStore` rather than raw provider history.
- **References**: `internal/gitops/rollback/{executor,git,argocd,k8s,register,engine,state,store_sqlite}.go`; `internal/gitops/verification/{result_store,result_store_sqlite}.go`; `internal/gitops/rollback/gitexec` (go-git, landed); `internal/gitops/rollback/argoexec` (REST, landed); `pkg/statemachine` (engine FSM); `pkg/dbutil` (SQLite stores); PROJECT-DETAILS §3.2 (K8s row — client-go deferred), §4.13 external-clients; Epic 16 tasks 7-9 (landed), forward 10; companions: the GitOps webhook receiver + cluster/blueprint/runbook boot-wiring entries.
