...
7. **Circuit breaker** state machine + tests. _(landed: real `circuitBreaker` replacing the noop stub from task 2; closed→open→half-open→closed with injectable clock for deterministic tests; `OnFailure`/`OnSuccess` driven from existing connect callbacks; `ConnectionManager.Health` returns error when every endpoint's breaker is OPEN. Active dial-time eviction (skipping OPEN endpoints when nats.go picks the next reconnect target) is **deferred to v0.x** — would require replacing nats.go's native multi-URL failover with a per-endpoint dial loop, marginal v1.0 benefit.)_
...
11. **`Manager.Health()` + `ConnectionManager.Health()`** integration. _(landed: per-endpoint observability moved to `pkg/natsstatus.EndpointSnapshot` (with pointer-or-nil timestamps + millisecond latencies for dashboard rendering); `internal/nats` uses type aliases. `NATSManager` interface gained `EndpointSnapshots()`; `/api/status` renders `nats_endpoints`. Down→up recovery tests at two layers: `TestHealthChecker_NATSDownThenUpRecovery` (deterministic toggling stub) + `TestManager_HealthDownThenUpRecovery` (tagged integration: kill standalone NATS, restart on same port, observe Manager.Health flip back to nil).)_
12. **IPv6 address bracket handling** in URL formatting (`[::]:4222` not `:4222`). _(landed: nats-server/v2 + nats.go already format IPv6 correctly when given a properly-bracketed URL — task 12 adds (a) `validateExternalURL` in `internal/config/nats.go` that catches the common operator typo `nats://::1:4222` (no brackets) at config load with a clear error pointing to PROJECT-DETAILS §4.2, and (b) IPv6 round-trip tests at both modes (`TestManager_EmbeddedIPv6Host` binds `::1`, `TestManager_ExternalIPv6URL` connects via `nats://[::1]:port`). Tests skip when the runner has no IPv6 stack. The `internal/netutil` lift of `pkg/api/server`'s listener helpers stays deferred — no current consumer in `internal/nats`.)_
13. **Integration test**: full embedded-NATS round-trip (publish + subscribe + JetStream consume). _(landed: `TestManager_JetStreamRoundTrip` in `manager_integration_test.go` drives the full Epic 05 wire path in one flow — SubjectBuilder produces the agent-command subject; `Manager.PublishEnvelope` marshals + dedups + publishes; the `KSCORE_COMMANDS_test` JetStream stream (Task 8) captures the message; a push consumer pulls it back; envelope round-trips intact (MessageID, CorrelationID, ClusterPrefix, Payload); dedup interceptor (Task 6) rejects a same-MessageID re-publish with `envelope.ErrDuplicate`; a fresh-MessageID publish succeeds. Push consumer used because pull is the agent's pattern and lands with Epic 06.)_

## Acceptance criteria

- [x] Embedded mode: `kscore-server` boots with NATS in-process; agents connect via `nats://localhost:4222`. _(real-binary boot covered by `cmd/kscore-server`'s `TestRun_BootsServesAndShutsDownCleanly`; agent-style publish + JetStream consume covered by Task 13's `TestManager_JetStreamRoundTrip`. The literal `:4222` in the bullet is illustrative — embedded mode dials in-process via `nats.InProcessServer`; external mode uses operator-configured URLs. Tests bind ephemeral free ports for parallel safety.)_
...
- [x] Health() reports unhealthy when NATS down; recovers when up. _(task 11 — `TestHealthChecker_NATSDownThenUpRecovery` covers the three-phase transition deterministically; `TestManager_HealthDownThenUpRecovery` (integration) drives a real kill+restart on the same port.)_
- [x] IPv6 endpoints work (`nats://[::1]:4222`). _(task 12 — `TestManager_ExternalIPv6URL` round-trips a publish over the bracketed URL form; `TestManager_EmbeddedIPv6Host` confirms `Embedded.Host = "::1"` produces a bracketed `ClientURL`. Config-load now rejects unbracketed IPv6 URLs with a §4.2-pointing error.)_
- [x] Coverage >80% on `internal/nats`. _(89.2% as of task 13; held above 85% for most of the epic.)_
