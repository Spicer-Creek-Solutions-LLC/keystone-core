...
- [x] `/health/live` always 200; `/health/ready` returns 503 during startup grace period. _(Task 6: `internal/health.Registry.Snapshot` aggregates NATS + DB + JetStream + custom checkers in parallel with a per-check timeout; `Ready` is `false` until uptime passes `StartupGracePeriod` (default 30s). The server's `/health/ready` handler returns 503 whenever `Ready=false`. Verified by `TestHealthChecker_InGracePeriodNotReady`, the Epic-04 health tests, and the new JetStream-fail test.)_
