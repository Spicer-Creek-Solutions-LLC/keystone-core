...
11. **`cmd/kscore-exec`** Cobra commands; output formatters (table, json, yaml). — 11a ✅ (`internal/cli/exec/`; per the operator's chosen split, subcommands live under `kscorectl exec` rather than a separate binary. `run` streams BatchExecuteCommand to terminal; `async` disconnects after the `batch_job_id` event; `script` reads a file and dispatches via `--shell bash` default. Persistent flags: `--server`, `--api-key`, `--output`. Dispatch flags: `--target`, `--concurrency`, `--command-timeout`, `--continue-on-failure`, `--shell`, `--working-dir`, `--user`, `--env`, `--job-id`. `BatchStreamRenderer` accumulates events and emits table / json / yaml; gRPC dial via insecure transport with `authorization: Bearer <api-key>` metadata. `ParseTarget` translates AND-of-labels-plus-hostname-glob client-side; full expression-string targeting → V1X.) — 11b deferred: `status`/`list`/`cancel`/`output`/`--dry-run` need proto additions.
