...
- `docs/project/CLI-REFERENCE.md` — every binary + every subcommand documented (auto-generated by `make docs-sync`).
- `docs/project/API-REFERENCE.md` — gRPC services + REST endpoints (auto-generated).
- `docs/project/CONFIGURATION-REFERENCE.md` — every config key with default + description (auto-generated).
- `docs/project/GETTING-STARTED.md` — 30-minute fresh-VM walkthrough (install, run a command, apply state).
- `docs/runbooks/` — backup-restore, bootstrap-new-cluster, capacity-scaling, security-incident.
- *(Hugo-rendered docs site at `docs/content/en/docs/…` is post-v1.0 and tracked under `docs/project/PUBLIC-LAUNCH-CHECKLIST.md` Phase G; v1.0 ships Markdown only.)*
...
_Originally drafted for v1.0.0. Re-scoped against the v0.1.0 release line
during v0.1.0 prep — lines met-for-v0.1.0 are ticked; lines inherently
v1.0-scope retain the [ ] and an annotation. The full v1.0.0 form of
each line ticks at v1.0.0 release time._

- [x] All v1.0 E2E scenarios pass in CI. _(landed: epic task 2a/2b/2c — 9 scenarios green on `ci-fast.yml`'s `e2e` job, required status check on `main`.)_
- [x] All HA resilience tests (Epic 13) pass. _(landed: in-process HA SLOs gate `make slo`; required status check on `main`.)_
- [x] All performance SLOs met in CI. _(landed: command latency <100ms, event throughput >10k/s, batch-10 fan-out <2s — `make slo` gates `main`.)_
- [x] Security baseline clean: 0 secrets, 0 known CVEs, 0 gosec high/critical. _(landed: epic task 7 — 4-scan baseline green on `ci-fast.yml`'s `security` job; per-finding rationale in `docs/project/SECURITY-GOVERNANCE.md`.)_
- [x] Coverage gates pass: critical >70%, CLI >40%. _(landed: epic task 4 — `make coverage-gate` enforces; allowList per-entry GRADUATE-BY notes in `tools/covgate/config.go`.)_
- [ ] No goroutine / connection / fd leaks in 1-hour soak test. _(v1.0-scope: `goleak` runs in every integration package — epic task 6 landed — but a dedicated 1-hour fd-leak soak harness is a v1.x ROADMAP entry per epic task 8 closeout. v0.1.0 ships the goroutine + connection audit table in `docs/project/HARDENING-BASELINE.md`.)_
- [x] All v1.0 binaries built for 5 platforms via `make release-snapshot`. _(landed: epic task 12 — 20 binaries × linux/darwin amd64+arm64 + windows amd64 = 5 archives + 12 nfpm packages + checksums.txt produced by `make release-snapshot`; gated by `make release-dry-run` on every PR.)_
- [x] SHA-256 checksum file generated. _(landed: `checksums.txt` is part of every `goreleaser` snapshot; verified by `make release-smoke`.)_
- [x] `kscorectl --help` lists all subcommands; each has help text. _(landed: dispatched binaries reachable as `kscorectl <name>`; `--help` audited by `make release-smoke`.)_
- [x] CLI reference docs cover every command. _(landed: epic task 9 — `docs/project/CLI-REFERENCE.md` auto-generated by `tools/gendocs/cli/`; drift gated by `make docs-sync-check`.)_
- [x] API reference docs cover every gRPC RPC + REST endpoint. _(landed: epic task 9 — `docs/project/API-REFERENCE.md` auto-generated from `.proto` + OpenAPI; drift gated by `make docs-sync-check`.)_
- [x] Configuration reference covers every key with default + description. _(landed: epic task 9 — `docs/project/CONFIGURATION-REFERENCE.md` AST-extracted from `internal/config/*.go`; drift gated by `make docs-sync-check`.)_
- [x] Quick-start guide can be followed end-to-end on a fresh Ubuntu VM in <30 minutes. _(landed: `docs/project/GETTING-STARTED.md` rewritten for the apt-install / systemctl / kscorectl operator path; matching ROADMAP closure entry.)_
- [ ] Backup → fresh cluster → restore round-trip identical state (verified by hash). _(v1.0-scope: Epic 18 acceptance lines for backup + restore are mechanism-proven in-process; a full operator-driven cluster-to-cluster round-trip is the v1.0 form. v0.1.0 ships `kscore-backup create|verify|restore` CLIs with the encrypted-artifact contract.)_
- [x] `RELEASE-PLAYBOOK.md`, `CHANGELOG.md` (v0.1.0 entry), `SECURITY.md`, `COMPATIBILITY.md` complete and reviewed. _(landed for v0.1.0: epic task 10 v1.0-version of `RELEASE-PLAYBOOK.md` covers v0.x line; `CHANGELOG.md` v0.1.0 entry hand-merged from 42 changie fragments + the epic-by-epic narrative; `SECURITY.md` audited for v0.1.0 unsigned posture; `COMPATIBILITY.md` covers supported-platform set.)_
- [ ] v1.0.0 tag pushed; goreleaser produces release artifacts. _(re-scoped: v0.1.0 tag is the active first-cut target. Tasks 14/15/16 — v1.0.0-rc1 / rc2 / final — are inherently v1.0.0 work and retarget to that release; v0.1.0 ships as a single direct cut per the soft-launch posture in `docs/project/PUBLIC-LAUNCH-CHECKLIST.md` F1.)_
