...
11. **`CHANGELOG.md` v0.1.0** entry — full feature list with epic mapping. _(landed — re-targeted from "v1.0.0" to v0.1.0 because v0.1.0 is the first public release per the versioning ladder in `docs/project/VERSIONING.md`; v1.0.0 entry is a separate future task that ships with the actual v1.0 cut. New v0.1.0 entry covers all 19 epics in dependency order, each with a deliverables summary linking the epic file. Sections: short release summary, audit-mode-only policy callout, Added (epic-mapped, ~19 entries with sub-bullets per epic), Known limitations (12 bullets naming v0.5/v1.0/v1.x graduation targets), Breaking changes (N/A — first release; future-release pattern documented), Migration (N/A — first release), Verification (single-signer flow inline + link to RELEASE-PLAYBOOK Appendix A v0.x row), Acknowledgments (sponsor + DCO/AI policy reference + Codeberg primary host). New `[v1.0.0] — Planned` stub above v0.1.0 captures the epic's stated intent without writing fiction. README "Releases" subsection added pointing at CHANGELOG, RELEASE-PLAYBOOK, VERSIONING.)_
12. **`.goreleaser.yaml` finalization** — all v1.0 binaries; multi-arch. _(landed — `.goreleaser.yaml` expanded from 3 binaries to all 20 (`cmd/kscore-*` + `kscorectl`). Build matrix: linux/darwin × amd64/arm64 + windows amd64 (windows arm64 dropped — uncommon in the operator audience). Five archives per release bundle every binary + LICENSE/NOTICE/README/CHANGELOG. Three nfpm package families: `kscore-server` + `kscore-agent` ship one binary + a systemd unit each; `kscore-cli` bundles `kscorectl` + every dispatched `kscore-*` sub-binary so a single `apt install kscore-cli` puts every subcommand on PATH. Six .deb + six .rpm = 12 native packages per release (3 families × 2 formats × 2 arches). New systemd units at `deploy/systemd/kscore-{server,agent}.service` (server hardened with NoNewPrivileges + ProtectSystem strict + RestrictNamespaces; agent intentionally less restricted since it executes operator-issued commands). New `make release` target wraps `goreleaser release --skip=publish --clean` for the playbook's offline-workstation step. Cross-platform build bugs fixed inline: `sysctl.normalizeValue` moved out of `provider_linux.go` into platform-agnostic `sysctl.go` (it's pure string handling); `file/fs.go` `syscall.Stat_t` ownership lookup split into `fs_unix.go` (linux/darwin/bsd via `//go:build unix`) and `fs_windows.go` (no-op shim setting UID/GID = -1). Verified end-to-end: `make release-snapshot` produces 4 tar.gz + 1 zip + 6 .deb + 6 .rpm + `checksums.txt`; each archive contains all 20 binaries; spot-checked binaries report correct `--version` + git commit + build date; one .deb installs cleanly in `debian:12-slim` and one .rpm in `rockylinux:9` with the systemd unit landing at `/lib/systemd/system/`.)_
13. **Release dry-run** — `make release-dry-run` produces artifacts; manual smoke-test of each. _(landed — `make release-dry-run` is now a full local validation: `release-config-check` (goreleaser check) → `release-snapshot` (goreleaser builds the 5 archives + 12 nfpm packages + checksums.txt) → `release-smoke` (`scripts/release-smoke.sh dist/`). The smoke script's host-side checks (checksum verify via `sha256sum -c`, archive content via `tar -tzf` / `unzip -Z1`) need only sha256sum + tar + unzip + docker; the linux-side checks (binary `--version` on all 20 cmd/kscore-* + dpkg-deb / rpm content listing on all 6 .deb + 6 .rpm) run inside `debian:12-slim` via `scripts/release-smoke-container.sh` (apt-installs rpm at container start) so the host has no rpm/dpkg-deb dependency. Opt-in `RELEASE_SMOKE_CONTAINERS=1` adds install smoke in fresh `debian:12-slim` (dpkg -i + --version + systemd unit present) and `rockylinux:9` (rpm -i + same assertions). New CI `release-dry-run` job runs the full smoke (`RELEASE_SMOKE_CONTAINERS=1`) on every PR. RELEASE-PLAYBOOK §Phase 4 step 3 now invokes `make release-dry-run` as the gating build step. The smoke surfaced + fixed inline: 15 operator-CLI binaries (audit, backup, blueprint, bootstrap, cluster, cluster-backup, events, files, gitops, identity, module, policy, runbook, secrets, webhook) that had no `--version` flag wired through their cobra root — extracted a shared `cli.AddVersion(*cobra.Command)` helper in `internal/cli/cli.go` that all 15 NewCommand constructors now call. All 20 binaries now report `<bin> <version>\ncommit: <sha>\nbuilt: <date>\n` consistently. New `docs/project/SECURITY-GOVERNANCE.md` "Release Dry-Run Smoke" section documents the gate + how-to-extend; ROADMAP entry "Release dry-run expansion" tracks the v1.x sub-bullets (SBOM, cross-arch install smoke, systemd-analyze verify).)_
