...
2. **All v1.0 E2E scenarios** wired into CI. _(2a landed — scenarios 1–3 over the single-topology harness from task 1: agent registration (PSK bootstrap + heartbeats reaching `connected`), single command exec via `ExecuteCommand`, and `ApplyState` round-trip. CI gains a non-blocking `e2e` job (`continue-on-error: true`) that runs `make e2e-test`. Required v1.0 wiring delivered as part of 2a: agent-side bootstrap-register publish on `Start`, server-side `AgentHeartbeat` subscriber routing to `ConnectionManager.Heartbeat`, bootstrap handler `OnAgentRegistered` callback feeding the conn-mgr cache, and `ListAgents`/`GetAgent` gRPC implementations. 2b landed — scenarios 4–6: `BlueprintService.ApplyBlueprint` against a distroless-compatible test catalog (`test/e2e/single/blueprints/e2e-noop`), multi-stdlib `ApplyState` proving the loader→runner chain (Option A — Option B / registry-based Starlark install+execute target deferred to v1.x pending gate-v1.0 "Module system boot wiring"), and KV-only `SecretsService` round-trip (Write/Get/List/Delete) against the encrypted-file backend (lease + transit are Vault-only in v1.0). Required v1.0 wiring delivered as part of 2b: `BlueprintService` registered at server boot with a filesystem catalog + server-local `Applier` (server-side apply only — remote-fleet dispatch remains the gate-v1.0 "Remote / distributed blueprint apply wiring" item), encrypted-file secrets backend enabled in test config, and an `e2e-noop` blueprint fixture next to the test code. **Pending**: 2c — scenarios 7–9 (audit query, outbound webhook, GitOps webhook+rollback) + flipping the CI job to required.)_
