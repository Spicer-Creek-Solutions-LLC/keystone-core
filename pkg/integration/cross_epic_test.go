...
func TestIntegration_EventReactorChain(t *testing.T) {
	...
	reactorSub, err := env.eventBus.SubscribeWithFilter("drift-reactor",
		...
		func(e *events.Event) error {
			...
			driftEvent := events.NewEvent(events.EventTypeStateDrift).
				...
				Build()
			env.eventBus.Publish(driftEvent)
			return nil
		})
	if err != nil {
	...
	if err := env.eventBus.Publish(triggerEvent); err != nil {
...
func TestIntegration_PolicyEvents(t *testing.T) {
	...
	if err := env.eventBus.Publish(passEvent); err != nil {
...
func TestIntegration_StateChangeEvents(t *testing.T) {
	...
	if err := env.eventBus.Publish(startEvent); err != nil {
	...
	if err := env.eventBus.Publish(changeEvent); err != nil {
	...
	if err := env.eventBus.Publish(doneEvent); err != nil {
