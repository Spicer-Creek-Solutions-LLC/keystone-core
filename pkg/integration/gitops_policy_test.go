...
func TestIntegration_GitOpsPolicyStateWorkflow(t *testing.T) {
	...
	if err := env.eventBus.Publish(gitopsEvent); err != nil {
	...
	if err := env.eventBus.Publish(policyEvalEvent); err != nil {
	...
	if err := env.eventBus.Publish(stateApplyEvent); err != nil {
	...
	if err := env.eventBus.Publish(stateCompleteEvent); err != nil {
...
func TestIntegration_GitOpsRollbackOnPolicyViolation(t *testing.T) {
	...
	reactorSub, err := env.eventBus.SubscribeWithFilter("rollback-reactor",
		...
		func(e *events.Event) error {
			...
			rollbackEvent := events.NewEvent(events.EventType("gitops.rollback.initiated")).
				...
				Build()
			env.eventBus.Publish(rollbackEvent)
			return nil
		})
	if err != nil {
	...
	if err := env.eventBus.Publish(deployEvent); err != nil {
	...
	if err := env.eventBus.Publish(violationEvent); err != nil {
...
func TestIntegration_PromotionPipeline(t *testing.T) {
	...
	for i, envName := range environments {
		...
		if err := env.eventBus.Publish(promotionEvent); err != nil {
