...
func TestIntegration_CommandRoutingInCluster(t *testing.T) {
	...
	if err := env.eventBus.Publish(cmdReceivedEvent); err != nil {
	...
	if err := env.eventBus.Publish(cmdRoutedEvent); err != nil {
	...
	if err := env.eventBus.Publish(cmdDeliveredEvent); err != nil {
	...
	if err := env.eventBus.Publish(cmdExecutedEvent); err != nil {
...
func TestIntegration_ClusterStateReplication(t *testing.T) {
	...
	if err := env.eventBus.Publish(stateChangeEvent); err != nil {
	...
	for _, server := range servers {
		...
		if err := env.eventBus.Publish(replicatedEvent); err != nil {
	...
	if err := env.eventBus.Publish(ackEvent); err != nil {
...
func TestIntegration_ClusterFailover(t *testing.T) {
	...
	if err := env.eventBus.Publish(failureEvent); err != nil {
	...
	if err := env.eventBus.Publish(electionEvent); err != nil {
	...
	if err := env.eventBus.Publish(newLeaderEvent); err != nil {
	...
	if err := env.eventBus.Publish(stabilizedEvent); err != nil {
...
func TestIntegration_MultiRegionCluster(t *testing.T) {
	...
	for _, region := range regions {
		...
		if err := env.eventBus.Publish(registerEvent); err != nil {
	...
	for _, gw := range gateways {
		...
		if err := env.eventBus.Publish(gatewayEvent); err != nil {
