...
path. The canonical repository is on Codeberg; the vanity domain is a
stable indirection so the public
...
- [`site/`](site/) — the generated HTML, committed so deploys
  don't need the `vangen` tool installed. Regenerated by hand; see
  below.
...
There is no `make` target for this. The Generation 1 root module carried the
`install-tools` and `vanity-regen` targets, and both went with it in reboot task
R08; this directory changes roughly never, so it is not worth a tool dependency
in the clean baseline.

```bash
go run 4d63.com/vangen@latest \\
  -config deploy/vanity/vangen.json -out deploy/vanity/site/
```
...
The cost (one Go binary, fetched on demand) is small compared to
