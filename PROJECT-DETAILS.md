...
- CLI: `kscore-cluster status|members|leader|add|remove|transfer-leader|rebalance|backup|restore`. `kscore-cluster-backup backup|restore|list|verify|schedule`. **Landed Epic 13 task 16** (`cmd/kscore-cluster/` + `cmd/kscore-cluster-backup/` over shared `internal/cli/cluster`; no new dep): all of `kscore-cluster` (`add` is an honest passthrough — AddMember is `Unimplemented` by contract) + `kscore-cluster-backup backup|restore` (remote, ClusterService gRPC) and `list|verify` (local snapshot-file inspection via `cluster.UnmarshalSnapshot`, server-free/CI-friendly). `schedule` (+ a `kscore-cluster watch` over the task-15 streams) deferred to the `v1.x` ROADMAP entry "kscore-cluster-backup schedule + kscore-cluster watch subcommands". kscore-policy CLI conventions (`--server`/`--api-key`/`-o`, insecure dial, `Deps{Dial}` bufconn-tested, 78.9% cov). Live use gated by the same deferred `gate-v1.0` boot-registration entry (ClusterService not yet registered at boot).
...
**Landed Epic 13 task 18** (`test/e2e/ha/slo_test.go`, `//go:build slo`; `make slo`; new `slo` CI job in `.woodpecker/ci.yml` + `.github/workflows/ci.yml`, every-PR-gated, no Postgres sidecar; no new dep): each SLO above measured against the real in-process cluster mechanisms (reusing the task-17 harness), asserted ≤ bound, measured value always logged. `make slo` runs **without `-race`** (race inflates wall-clock 2–10× — the in-`-race` functional smoke is the task-17 `make test-integration` suite). Honest scope: these verify the *mechanism* SLOs; the server-integrated numbers (agent-reconnect latency, graceful-shutdown zero-disconnect timing) ride with the `gate-v1.0` "HA E2E multi-process / iptables-partition form" ROADMAP entry. **Epic 13 closed** — mechanisms complete + SLO-verified; server-boot-integration carried by four `gate-v1.0` ROADMAP entries.
