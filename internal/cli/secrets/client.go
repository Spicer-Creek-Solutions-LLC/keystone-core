...
// connection — mTLS for the CLI is tracked under a follow-up
// ROADMAP entry alongside the kscore-identity CLI's mTLS gap.
// API-key auth flows through the `authorization: Bearer <key>`
// metadata per pkg/api/auth.APIKeyAuthenticator.
func dialGRPC(_ context.Context, target, _ string) (v1.SecretsServiceClient, io.Closer, error) {
	if target == "" {
		return nil, nil, fmt.Errorf("secrets: --server is required")
	}
	conn, err := grpc.NewClient(target,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
	)
	if err != nil {
		return nil, nil, fmt.Errorf("secrets: dial %s: %w", target, err)
	}
