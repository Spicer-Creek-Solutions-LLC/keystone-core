...
// connection — mTLS for the CLI is a v0.x ROADMAP carry-over shared
// with kscore-identity + kscore-secrets. API-key auth flows through
// the `authorization: Bearer <key>` metadata.
func dialGRPC(_ context.Context, target, _ string) (v1.EventServiceClient, io.Closer, error) {
	if target == "" {
		return nil, nil, fmt.Errorf("events: --server is required")
	}
	conn, err := grpc.NewClient(target,
		grpc.WithTransportCredentials(insecure.NewCredentials()),
	)
	if err != nil {
		return nil, nil, fmt.Errorf("events: dial %s: %w", target, err)
	}
