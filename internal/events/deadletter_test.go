...
func TestDeadLetterAlert(t *testing.T) {
	...
	var receivedAlert atomic.Pointer[DeadLetterAlert]

	config := DefaultDeadLetterConfig()
	config.Path = ":memory:"
	config.AutoRetry = false
	config.AlertThreshold = 5
	alertCh := make(chan *DeadLetterAlert, 1)
	config.AlertCallback = func(alert *DeadLetterAlert) {
		alertReceived.Store(true)
		alertCopy := *alert
		receivedAlert.Store(&alertCopy)
		alertCh <- &alertCopy
	}
	...
	if alert := receivedAlert.Load(); alert != nil {
		if alert.PendingCount < 5 {
			t.Errorf("Expected pending count >= 5, got %d", alert.PendingCount)
		}
