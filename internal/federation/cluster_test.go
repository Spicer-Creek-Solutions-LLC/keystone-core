...
func TestNewFederation_TLSConfig(t *testing.T) {
	store := NewInMemoryClusterStore()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			transport, ok := fed.httpClient.Transport.(*http.Transport)
			if !ok || transport == nil {
				t.Fatal("Expected custom http transport to be set")
			}
			if transport.TLSClientConfig == nil {
				t.Fatal("Expected TLSClientConfig to be set")
			}
			if transport.TLSClientConfig.InsecureSkipVerify != tt.skipVerify {
				t.Errorf("InsecureSkipVerify = %v, want %v", transport.TLSClientConfig.InsecureSkipVerify, tt.skipVerify)
			}
			if transport.TLSClientConfig.MinVersion != tt.wantMinVersion {
				t.Errorf("MinVersion = %v, want %v", transport.TLSClientConfig.MinVersion, tt.wantMinVersion)
			}
		})
	}
}
