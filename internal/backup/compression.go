...
func (c *Compressor) Compress(ctx context.Context, r io.Reader, w io.Writer) error {
	...
	//nolint:gosec // G204: compression command execution is intentional for backup system
	cmd := exec.CommandContext(ctx, info.Command, args...) // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command
	cmd.Stdin = r
	cmd.Stdout = w
...
func (c *Compressor) Decompress(ctx context.Context, r io.Reader, w io.Writer) error {
	...
	if c.compressionType == CompressionTypeGzip {
		...
		defer gzr.Close()
		_, err = io.Copy(w, gzr) //nolint:gosec // G110: backup artifacts are generated by trusted workflows and may exceed fixed limits; nosemgrep: go.lang.security.decompression_bomb.potential-dos-via-decompression-bomb
		return err
	}
	...
	//nolint:gosec // G204: decompression command execution is intentional for backup system
	cmd := exec.CommandContext(ctx, info.DecompCmd, args...) // nosemgrep: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command
	cmd.Stdin = r
	cmd.Stdout = w
