...
func (s *SyslogOutput) buildTLSConfig() (*tls.Config, error) {
	...
	minVersion, err := parseTLSMinVersion(s.config.TLS.MinVersion)
	if err != nil {
		return nil, err
	}
	...
	for attempt := 0; attempt <= s.config.MaxReconnectAttempts || s.config.MaxReconnectAttempts == 0; attempt++ {
		...
		if s.conn == nil {
			if err := s.connect(); err != nil {
				...
				if !s.waitForReconnect(s.config.ReconnectInterval) {
					return fmt.Errorf("syslog output is closed")
				}
				continue
			}
...
func (s *SyslogOutput) waitForReconnect(delay time.Duration) bool {
	if delay <= 0 {
		return true
	}
