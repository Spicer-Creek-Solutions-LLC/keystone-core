...
func TestClusterConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{"coord listen_addr empty ok", func(c *ClusterConfig) { c.Coordination.ListenAddr = "" }, ""},
		{"coord listen_addr valid host:port ok", func(c *ClusterConfig) { c.Coordination.ListenAddr = "0.0.0.0:9443" }, ""},
		{"coord listen_addr bind-all port ok", func(c *ClusterConfig) { c.Coordination.ListenAddr = ":9443" }, ""},
		{"coord listen_addr missing port", func(c *ClusterConfig) { c.Coordination.ListenAddr = "0.0.0.0" }, "listen_addr must be host:port"},
		{"coord listen_addr missing port after colon", func(c *ClusterConfig) { c.Coordination.ListenAddr = "0.0.0.0:" }, "must include a port"},
