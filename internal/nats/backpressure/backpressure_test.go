...
func TestPublisher_Drop(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		return publisher.Stats().Pending > 0, nil
	}); err != nil {
		t.Fatalf("Expected pending messages before release: %v", err)
	}
...
func TestAdaptiveFlowController(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(200*time.Millisecond, 5*time.Millisecond, func() (bool, error) {
		return afc.WindowSize() <= initialWindow, nil
	}); err != nil {
		t.Fatalf("Window should decrease after failure: %v", err)
	}
}
