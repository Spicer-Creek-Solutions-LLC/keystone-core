...
func TestNewTLSBaseConfig(t *testing.T) {
	config := newTLSBaseConfig("service.local", "")

	if config.ServerName != "service.local" {
		t.Errorf("ServerName = %q, want %q", config.ServerName, "service.local")
	}
...
func TestNewTLSBaseConfig_MinVersionOverride(t *testing.T) {
	config := newTLSBaseConfig("service.local", "1.2")
