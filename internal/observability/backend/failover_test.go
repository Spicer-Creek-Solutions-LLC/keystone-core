...
func TestManager_Write(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		return len(backend.Data()) == 10, nil
	}); err != nil {
		t.Fatalf("Timed out waiting for queued writes: %v", err)
	}
...
func TestManager_Failover(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		return len(secondary.Data()) > 0, nil
	}); err != nil {
		t.Fatalf("Secondary should have received data after failover: %v", err)
	}
}
...
func TestManager_LeastLatency(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		...
		t.Fatalf("Fast backend should have more writes: fast=%d, slow=%d", fastCount, slowCount)
	}
}
...
func TestManager_HealthCheck(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		...
		t.Fatalf("Expected health_check_failed event: %v", err)
	}
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		...
		t.Fatalf("Expected health_check_recovered event: %v", err)
	}
}
...
func TestManager_Concurrent(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		return len(backend.Data()) == 1000, nil
	}); err != nil {
		t.Fatalf("Backend received %d items, want 1000", len(backend.Data()))
	}
}
...
func TestManager_RetryWithBackoff(t *testing.T) {
	...
	if err := helpers.WaitForTimeout(2*time.Second, 10*time.Millisecond, func() (bool, error) {
		return len(backend.Data()) >= 2, nil
	}); err != nil {
		t.Fatalf("Expected at least 2 successful writes, got %d", len(backend.Data()))
	}
}
...
func TestMemoryBackend_Write_ContextCancel(t *testing.T) {
	backend := NewMemoryBackend("test")
	backend.SetLatency(200 * time.Millisecond)

	ctx, cancel := context.WithTimeout(context.Background(), 50*time.Millisecond)
	defer cancel()

	err := backend.Write(ctx, []byte("data"))
	if err == nil {
