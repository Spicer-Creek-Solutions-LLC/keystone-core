...
func (p *linuxProvider) CreateBridge(ctx context.Context, s BridgeSpec) error {
	bin, err := p.ip()
	if err != nil {
		return err
	}
	args := []string{"link", "add", s.Name, "type", "bridge"}
	if s.STP {
...
func execRun(ctx context.Context, bin string, args []string) (string, error) {
	cmd := exec.CommandContext(ctx, bin, args...) //nolint:gosec // bin resolved via exec.LookPath; args are fixed iproute2 flags + a validated interface name from a validated declaration
