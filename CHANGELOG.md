...
- **Tooling to build signed APT and DNF/YUM package repositories.** New
  ...
  **docker or podman** when absent on the host, so the whole flow runs
  ...
  `apt-get install kscore-cli=<ver>`), verifies signatures, and uploads
  with `--delay-updates` for a near-atomic switchover, refusing to publish
  a test-key or unsigned tree. Leading the getting-started guide with the
  `apt install` / `dnf install` path and per-package RPM signing remain
  follow-ups.
...
- **Release signing deferred to v0.8; v0.1–v0.7 ship unsigned.** The
  ...
  artifacts, signed-commit enforcement, and hosted signed apt/dnf repos)
  becomes the **v0.8 supply-chain milestone**, landing as one
  key-onboarding batch rather than gating each earlier v0.x cut — notably
  the v0.5 external-tester milestone, which therefore has no remaining
  release blockers. Documented across `RELEASE-PLAYBOOK.md` §6 + the
  release-lines table, `SECURITY.md`, `VERSIONING.md`, and the three
  `ROADMAP.md` signing entries.
